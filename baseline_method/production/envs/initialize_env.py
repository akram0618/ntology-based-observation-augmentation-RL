...
PATH_TIME = "D:/SimRL - Paper2 - baseline/log/" + datetime.now().strftime("%Y%m%d_%H%M%S")
...
def extend_agent_parameters(parameters):
    ...
    #parameters.update({'TRANSP_AGENT_STATE': ['rel_buffer_fill_in_out', 'bin_machine_failure', 'total_process_time']})  # Alternatives: bin_buffer_fill, bin_machine_failure, bin_location, int_buffer_fill, rel_buffer_fill, rel_buffer_fill_in_out, order_waiting_time, order_waiting_time_normalized, distance_to_action, remaining_process_time, total_process_time
    parameters.update({'TRANSP_AGENT_STATE': ['total_process_time']})
...
def extend_production_parameters(parameters):
    ...
    # Source parameters
    parameters.update({'SOURCE_CAPACITIES': [12] * parameters['NUM_SOURCES']})  # Number of load ports
    ...
    parameters.update({'MACHINE_CAPACITIES': [3] * parameters['NUM_MACHINES']})  # Capacity for in and out machine buffers together
