...
class Transport(Resource):
    ...
    def calculate_state(self):
        ...
        if 'bin_machine_failure' in self.parameters['TRANSP_AGENT_STATE']:
            ...
            rand_num = random.sample(range(0, 8), 4)
            for i in rand_num:
                if mach.last_broken_start > mach.last_process_start:
                    state[i] = True
                else:
                    state[i] = False
        ...
        if 'total_process_time' in self.parameters['TRANSP_AGENT_STATE']:
            ...
            for mach in self.resources['machines']:
                ...
                rand_num = random.sample(range(0, 8), 4)
                for i in rand_num:
                    mach = self.resources['machines'][i]
                    temp_stt = state[i]
                    if self.resources['machines'][i].buffer_processing != None:
                        state[i] = len(mach.buffer_in) * self.parameters['AVERAGE_PROCESS_TIME'][mach.id] + mach.last_process_time - (self.env.now - mach.last_process_start)
                    else:
                        state[i] = len(mach.buffer_in) * self.parameters['AVERAGE_PROCESS_TIME'][mach.id]
    ...
    def transporting(self):
        while True:
            ...
            #print("tardy order: ", order , "  " , order_tardiness)
            if self.parameters['PRINT_CONSOLE']:
                print("Order state")
